...
pub struct Metrics {
    ...
    label_count_by_label_name: Family<DefaultLabels, Gauge>,
...
pub fn initialize_registry(mut registry: Registry, config: Config) -> Result<AppState, Error> {
    let metrics = Metrics {
        ...
        label_count_by_label_name: Family::default(),
    ...
    registry.register(
        ...
        metrics.label_count_by_label_name.clone(),
    );
    registry.register(
    ...
    for instance in &state.config.prometheus.instances {
        match fetch_metrics(&instance.url, instance.limit).await {
            Ok(metrics) => {
                ...
                for metric in &metrics.data.label_count_by_label_name {
                    let default_labels = &DefaultLabels {
                        instance: instance.name.clone(),
                        name: metric.name.clone(),
                    };
